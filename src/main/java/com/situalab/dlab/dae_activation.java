...
public class dae_activation implements Serializable {
    ...
    static activateV<Double> dsigmoid = (activateV<Double> & Serializable) xoz -> dae_activation.sigmoid.apply(xoz) * (1 - dae_activation.sigmoid.apply(xoz));


    //tanh
    static activateV<Double> ftanh = (activateV<Double> & Serializable) xoz -> ((exp(xoz)-exp(-xoz)) / (exp(xoz)+exp(-xoz)));
    static activateV<Double> dtanh = (activateV<Double> & Serializable) xoz -> (1-Math.pow(dae_activation.ftanh.apply(xoz),2));
